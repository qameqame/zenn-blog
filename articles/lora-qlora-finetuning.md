...
つまり実態としては「まずプロンプトで済ます → 情報が足りなければRAGを追加 → それでも解決しない振る舞いの問題にのみFine-tuning」という優先順位です。Fine-tuningはフロンティアチームが使うニッチな技術であり、「RAGとFine-tuningのハイブリッドが標準」というのは現状の実態とは異なります。
...
- プロンプトだけでは一貫したスタイル・トーンを保てない
- 高頻度クエリ（1日10万件以上）でRAGの推論コストを下げたい
- インターネット接続のないオフライン・エアギャップ環境での運用
