---
title: "MCPツールの「検索コントラクト」を明示する — Dev.toのフィードバックから学んだこと"
emoji: "📋"
type: "tech"
...
先日、pgvectorの検索機能をMCPサーバーとして公開する記事を[Dev.to](https://dev.to/hiroki-kameyama/building-a-rag-system-from-scratch-mcp-exposing-pgvector-as-a-reusable-tool-server-2onc)に投稿したところ、AIEmployees & Terminal Skillsの創業者であるAlex Shev氏から以下のコメントをいただきました。
...
本記事では、このフィードバックを受けて「検索コントラクト」とは何か・なぜ重要か・どう実装するかを整理します。
...
MCPでサーバー化することで「検索機能がインフラになった」というのがAlex氏の評価です。そのうえで「インフラとして使うなら、呼び出し側への約束（コントラクト）を明示すべき」という指摘です。
...
Alex氏のコメントに「Evals much more tractable」という返答をしましたが、これは実際に重要なポイントです。
...
Dev.toへの英語発信で、実装者の視点では気づかなかった設計観点をいただきました。
...
「MCPでツールをサーバー化する」だけでなく「そのツールの動作を明示的に約束する」ことが、本番で信頼できるシステムを作る上で重要だと改めて学びました。
...
- [Alex Shevのコメント](https://dev.to/hiroki-kameyama/building-a-rag-system-from-scratch-mcp-exposing-pgvector-as-a-reusable-tool-server-2onc#comment-3a7jf)
